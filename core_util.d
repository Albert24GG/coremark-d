...
else static if (SEED_METHOD == SEED_ARG)
{
...
ee_u16 crcu8(ee_u8 data, ee_u16 crc)
{
    ...
    for (i = 0; i < 8; i++)
    {
        x16 = cast(ee_u8)((data & 1) ^ (cast(ee_u8) crc & 1));
        data >>= 1;
...
{
    crc = crcu8(cast(ee_u8)(newval), crc);
    crc = crcu8(cast(ee_u8)((newval) >> 8), crc);
    return crc;
}

ee_u16 crcu32(ee_u32 newval, ee_u16 crc)
{
    crc = crc16(cast(ee_s16) newval, crc);
    crc = crc16(cast(ee_s16)(newval >> 16), crc);
    return crc;
}

ee_u16 crc16(ee_s16 newval, ee_u16 crc)
{
    return crcu16(cast(ee_u16) newval, crc);
}
...
ee_u8 check_data_types()
{
    ...
    if (ee_ptr_int.sizeof != (int*).sizeof)
    {
        ee_printf(
